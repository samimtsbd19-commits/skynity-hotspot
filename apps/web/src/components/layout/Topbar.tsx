...
import { Search, Bell, LogOut, User, Globe, Menu } from "lucide-react";
...
export default function Topbar({ onMenuClick }: TopbarProps) {
  ...
  return (
    <header className="h-16 border-b border-[rgba(0,234,255,0.15)] bg-[#0A1628]/80 backdrop-blur-md flex items-center justify-between px-4 md:px-6 sticky top-0 z-40">
      <div className="flex items-center gap-3 flex-1">
        <button
          onClick={onMenuClick}
          className="md:hidden p-2 rounded-lg hover:bg-[#112240] text-sky-text-secondary transition-colors"
        >
          <Menu size={20} />
        </button>
        <div className="flex items-center gap-3 flex-1 max-w-md">
          <Search size={18} className="text-sky-text-secondary hidden sm:block" />
          <input
            type="text"
      ...
      <div className="flex items-center gap-2 md:gap-4">
        ...
        <div className="flex items-center gap-2 pl-2 md:pl-4 border-l border-[rgba(0,234,255,0.15)]">
